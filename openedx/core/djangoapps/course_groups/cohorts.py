...
def remove_user_from_cohort(cohort, username_or_email):
    ...
    try:
        ...
        COHORT_MEMBERSHIP_UPDATED.send(sender='COHORT_MEMBERSHIP_UPDATED_Remove', user=user, course_key=course_key)
    except CohortMembership.DoesNotExist:
...
def add_user_to_cohort(cohort, username_or_email):
    ...
    try:
        ...
        course_key = membership.course_id
        # _invalidate_cache(user.id, course_key)
        COHORT_MEMBERSHIP_UPDATED.send(sender='COHORT_MEMBERSHIP_UPDATED', user=user, course_key=course_key)
        tracker.emit(
